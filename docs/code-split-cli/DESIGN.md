# Technical Design — Code Split CLI (`code-split-cli`)

The technical design of the `code-split` binary: the `code-split-cli` crate
...
viewer assets see [`code-split-viewer/DESIGN.md`](../code-split-viewer/DESIGN.md).

<!-- toc -->

- [1. Component Model](#1-component-model)
  - [code-split-cli](#code-split-cli)
  - [code-split-cli recommendation engine](#code-split-cli-recommendation-engine)
...
- **`report`** (`run_report`): runs the shared analysis core (analyzing the
  ...
  use `--top`). See [§1 `code-split-cli` recommendation engine](#code-split-cli-recommendation-engine).
...
Interfaces are defined in [`code-split-cli/PRD.md`](PRD.md) (and the main PRD §7).
This section notes the implementation binding.

### Unified CLI

`cpt-code-split-interface-cli`
...
`cpt-code-split-interface-report-cli`
...
`cpt-code-split-interface-check-cli`
