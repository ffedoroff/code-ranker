...
| Snapshot shows `"branch": "HEAD"`, a merge-commit hash, an inflated `dirty_files`, or no/odd `origin` | Actions' detached checkout (and the merge ref on PRs) mangle the raw `git` view | Map Actions variables onto the `--git.*` flags (already wired in both reference files) — see [`docs/CLI.md` → Git metadata overrides](../../../CLI.md#git-metadata-overrides) |
