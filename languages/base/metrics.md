...
**TL;DR**: Code Ranker classifies every physical line of a source file into
exactly one LOC bucket — `sloc`, `cloc`, `blank`, or `tloc`. Inline tests are
split off *first* (test-annotated items), so they never inflate the production
size, HK, or complexity of a file. The walk-through below counts a 14-line
hello-world by hand.

## What "correct" means (normative)

This file is the **source of truth** for *what each metric counts* — the
...
- **Counted from the parsed syntax tree, never from text.** A metric keyword that
  appears only as a look-alike — inside an identifier (`super_unsafe_fn`), a
  comment, a string / char literal, a doc-comment, an attribute, or an unexpanded
  macro body — **does not count**. No false positives from text.
- **Production code only.** Test-annotated items are stripped first, so nothing
  inside them counts toward any production metric (their lines become `tloc`).
- **Generated/macro code is not expanded.** A construct produced *inside* a macro
  or code-generation invocation is invisible — a deliberate non-goal, not a missed
  count.

**Keyword look-alike guard set.** These are the construct keywords/operators a
complexity or `unsafe`-style metric can key on; the FP tests inject each *only* as
a look-alike (comment / string / identifier / …) and assert no metric moves. The
...
`?`. (The per-language false-positive matrix in `code-ranker-plugins`'s tests
iterates this exact list, and a test asserts the list it uses is documented here —
...
per-construct increment rules for these follow the algorithm of record that the
in-tree engine replicates; the guarantees this spec adds on top are the three
rules above plus summation and omission. Reconciling the analyzer's definitions
against independent tools is the differential layer in `docs/metric-correctness.md`.
...
// Greet the world.                             // | 1  |      |  x   |       |      |
function greet() -> string {                    // | 2  |  x   |      |       |      |
    return "hello, world"                       // | 3  |  x   |      |       |      |
}                                               // | 4  |  x   |      |       |      |
                                                // | 5  |      |      |   x   |      |
@test                                           // | 6  |      |      |       |  x   |
module tests {                                  // | 7  |      |      |       |  x   |
    import greet from parent                    // | 8  |      |      |       |  x   |
    // check the greeting                       // | 9  |      |      |       |  x   |
                                                // | 10 |      |      |       |  x   |
    @test                                       // | 11 |      |      |       |  x   |
    function greets() {                         // | 12 |      |      |       |  x   |
        assert greet() == "hello, world"        // | 13 |      |      |       |  x   |
    }                                           // | 14 |      |      |       |  x   |
}                                               // | 15 |      |      |       |  x   |
```
...
the test annotation through the module's closing brace is test code
...
| `tloc` | **10** | Test lines — the *whole* test region (lines 6–15), annotation, comment, blank, and braces included. |
...
The pass strips test-annotated items (via a syntax-tree walk) *before* measuring
anything. So the production source the analyzer actually sees is just lines 1–5:
...
- A configuration flag merely *named* "test" (e.g. a feature toggle) is **not** a
  test — only a genuine test annotation / test-build identifier triggers the
  split. A string literal `"test"` is left as production code.
...
two sources: the **in-tree per-language engine** (complexity, Halstead, MI) and
the **dependency graph** (`fan_in` / `fan_out` / `hk`).
...
point**. The analyzer counts each of: a conditional, a loop (`for`/`while`/
general loop), a branch arm, an error-propagation operator, `&&`, `||`. No
branches → `1`.

```
function classify(n) -> string {        //   base                 = 1
    if n < 0 {                          //   if                  +1  → 2
        return "neg"
...
which equals `Σ` over functions of `(E − N + 2)`; the algorithm of record models
the file itself as one more unit space with a base path of `1` and reports
`cyclomatic_sum()` = that file unit `+ Σ over functions`. We emit the analyzer's
value verbatim — it is the analyzer's definition of the file's complexity, and it
is the same value fed into `mi` (below), so the two stay coherent. A file with no
functions (a pure type or declaration block) is left with only the file unit's
...
> theory the per-function sum carries no container term; the algorithm of record
...
same branch nested two levels deep costs 1 + 2 = 3. A flat sequence of
conditionals is cheap; a deeply nested pyramid is expensive even at the same
cyclomatic count. Linear code, early returns, and `&&`/`||` chains add little;
`break`/`continue` to a label and re-nesting add a lot. Like `cyclomatic`, the
file-level value is the **sum over every function**; a function-less file omits
it.
...
- **`exits`** — the number of *possible exit points* of a function: each explicit
  `return` and each error-propagation operator, **plus the function's normal
  value-returning exit** (a function that declares a return type exits at
  least once with a value). "Number of exit points" is not a McCabe/Halstead
  metric and has no single canonical definition; this is the rule the algorithm
  of record defines and our engine ports, and it is the source of truth here.
  Consequence: a one-line function `function f() -> int { return 1 }` reports
  `exits = 2` (the explicit `return` plus the value-returning exit), and a
  function with no declared return type and no `return` reports `0`. The
  return-type-gated normal exit is a deliberate analyzer approximation, documented
  rather than "corrected" (a precise count would need real control-flow analysis
  the analyzer does not do).
- **`args`** — parameter count, summed over functions **and** closures.
- **`closures`** — number of closures / anonymous functions defined.

A function-less file omits all three (their no-signal value is `0`). As with the
others, a `return` / error-propagation operator / closure marker appearing in a
comment, string, or identifier does not count — these are syntax-tree counts, not
text matches.

### Halstead — the operator/operand dictionaries

This is where the **dictionaries** come in. The engine walks the
syntax tree and fills two maps:

- **operators** — keyed by token *kind* (`+`, `=`, `if`, `(`, `function`, …).
  Counts how many distinct operators appear and how often.
...
edges (real import/path dependencies — re-exports are excluded),
...
that defines the item (following re-exports). Then only edges of a
**flow kind** are tallied: of the edge kinds the plugin emits, just
`uses` is `flow: true` — `contains` (the module-ownership tree), `reexports`
(re-export facades), and `super` (glob pulls from an ancestor) are all
`flow: false` and never reach `fan_in` / `fan_out` / `hk`. So "resolves to X" and
"counts toward coupling" are separate gates; an edge must pass both.

Worked on `parser`. First, what `parser` itself imports (`fan_out`):

```
// parser
import Node, Expr from ast      // uses → ast            → fan_out +1
import Token from lexer         // uses → lexer          → fan_out +1
import Stmt from ast            // uses → ast (same file) — already counted
import Serialize from json_lib  // uses → json_lib, but external → fan_out_external, not fan_out
                                //                       → fan_out = 2
```

`ast` and `lexer` are two distinct internal files, so `fan_out = 2`. The
imports resolve to the *defining file*, so `Node`, `Expr`, and `Stmt` (all
defined in `ast`) collapse to one partner — partners are **unique**, so
...
separately.) `json_lib` resolves fine and its edge is even `flow: true`, but the
target is an external library, so HK routes it to `fan_out_external` rather than
`fan_out`.

Now, who depends on `parser` (`fan_in`) — looking across the other files:

```
// module index — declares the module
declare module parser            // contains  → parser  — flow: false, NOT counted (structure)

// package facade
re-export Parser from parser     // reexports → parser  — flow: false, NOT counted (facade)

// repl
import Parser from parser        // uses      → parser  → counts for parser: fan_in +1
import parse from parser         // uses      → parser (same file) — already counted

// main
import parse from parser         // uses      → parser  → counts for parser: fan_in +1
                                 //                      → parser fan_in = 2
```

Only the two `uses` edges count, so `parser` has `fan_in = 2` (from `repl`
and `main`). The other two edges into `parser` resolve to it but are
non-flow and dropped: the module declaration in the index is a `contains` edge
(module ownership — structure, not a dependency), and the re-export in the
package facade is a `reexports` edge (a facade). This is exactly why hub files
like package indexes and facades don't accumulate false coupling. `repl`'s second
import is another `uses` edge to the same file, so the uniqueness rule collapses
it to one.
...
Dependencies on **external libraries** (standard library, third-party packages)
are *not* counted toward `fan_out`. They're tracked apart because we measure how
coupled a module is *within this codebase* — those are the edges you can actually
refactor. A dependency on a third-party serialization library is a fixed cost; a
dependency on a sibling module is a design choice that drives `hk` and splitting
decisions.
...
(in its in-tree per-language engines) over the production remainder.
...
- **Halstead** (`vocabulary`, `length`, `volume`, `effort`, `time`, `bugs`) —
  ...
  discriminations per delivered bug) are Halstead's empirical values.
