//! The report list-override DSL: parse the `<lang>.toml` `[report]` section into
//! a [`ReportOverride`] of per-list [`ListPatch`]es.
//!
//! The report's table `columns`, card-featured metrics, and JSON `stats` keys are
//! inherited from the global metric catalog (`code-ranker-graph/metrics/builtin.toml`).
//! A language patches an inherited list rather than restating it: a plain array
//! replaces it wholesale, while an **op-table** mutates it in place —
//!
//! ```toml
//! [report]
...
//!
//! The same primitive is reused by `config::deep_merge`: an inherited list in
//! `defaults.toml` can be patched (not just replaced) by a `<lang>.toml`
//! op-table, via [`is_list_op_table`] + [`patch_value_list`].

use code_ranker_plugin_api::report::{ListPatch, ReportOverride};
...
pub(crate) fn is_list_op_table(t: &Table) -> bool {
...
pub(crate) fn patch_value_list(base: Vec<Value>, ov: &Value) -> Vec<Value> {
...
#[cfg(test)]
#[path = "tests/list_override.rs"]
mod list_override_tests;
